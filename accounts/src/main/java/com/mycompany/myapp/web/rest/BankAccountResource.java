...
public class BankAccountResource {
    ...
    private static final String ENTITY_NAME = "accountsBankAccount";
    ...
    public ResponseEntity<BankAccount> partialUpdateBankAccount(
        ...
        Optional<BankAccount> result = bankAccountRepository
            ...
            .map(existingBankAccount -> {
                if (bankAccount.getLogin() != null) {
                    existingBankAccount.setLogin(bankAccount.getLogin());
                }
    ...
    public ResponseEntity<List<BankAccount>> getAllBankAccounts(@org.springdoc.core.annotations.ParameterObject Pageable pageable) {
        ...
        if (SecurityUtils.isCurrentUserInRole("ROLE_ADMIN")) {
            // Admins can retrieve all customers
            page =bankAccountRepository.findAll(pageable);
        } else {
            // Regular users can only retrieve their own data
            page = bankAccountRepository.findByLogin(currentUserLogin, pageable);
        }
